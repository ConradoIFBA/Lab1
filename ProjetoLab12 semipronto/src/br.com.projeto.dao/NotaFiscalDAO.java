...
public class NotaFiscalDAO {
    ...
    public void inserir(NotaFiscal notaFiscal) throws Exception {
        ...
        String sql = "INSERT INTO nota_fiscal (numero, data_emissao, valor, vendas_id, usuario_id) VALUES (?, ?, ?, ?, ?)";
    ...
    public List<NotaFiscal> listar() throws Exception {
        ...
        // ✅ CORRIGIDO: Nomes de colunas em snake_case
        String sql = "SELECT * FROM nota_fiscal WHERE ativo = true ORDER BY data_emissao DESC";
    ...
    public List<NotaFiscal> listarPorUsuario(int usuarioId) throws Exception {
        List<NotaFiscal> notas = new ArrayList<>();
        // ✅ CORRIGIDO: Nomes de colunas em snake_case
        String sql = "SELECT * FROM nota_fiscal WHERE usuario_id = ? AND ativo = true ORDER BY data_emissao DESC";
        PreparedStatement stmt = conexao.prepareStatement(sql);
    ...
    public void excluir(int id) throws Exception {
        // Exclusão lógica
        // ✅ CORRIGIDO: Nomes de colunas em snake_case
        String sql = "UPDATE nota_fiscal SET ativo = false WHERE id_nota_fiscal = ?";
        PreparedStatement stmt = conexao.prepareStatement(sql);
        stmt.setInt(1, id);
        stmt.executeUpdate();
        stmt.close();
    ...
    public NotaFiscal buscar(int id) throws Exception {
        // ✅ CORRIGIDO: Nomes de colunas em snake_case
        String sql = "SELECT * FROM nota_fiscal WHERE id_nota_fiscal = ? AND ativo = true";
        PreparedStatement stmt = conexao.prepareStatement(sql);
        stmt.setInt(1, id);
        ResultSet rs = stmt.executeQuery();
    ...
    public NotaFiscal buscarPorVenda(int vendasId) throws Exception {
        // ✅ CORRIGIDO: Nomes de colunas em snake_case
        String sql = "SELECT * FROM nota_fiscal WHERE vendas_id = ? AND ativo = true";
        PreparedStatement stmt = conexao.prepareStatement(sql);
